...
**성과**
- 매 프레임 손 랜드마크를 `List<List<float>>` 로 새로 만들던 경로를 **손 데이터 객체 풀링**으로 바꿔 — 값만 덮어쓰고 구조는 재사용해 **프레임당 할당을 사실상 없앴습니다**
- 워커 → 메인 스레드 브릿지의 **큐에 상한**을 걸어, 메인 스레드가 밀리면 오래된 프레임부터 버립니다 — **입력 지연이 쌓여 손과 화면이 어긋나는 현상 차단**
- 비싼 **룰 매칭은 새 데이터가 올 때만**, 시간 디바운스는 **매 프레임** 으로 주기를 갈라 연산 낭비 제거
