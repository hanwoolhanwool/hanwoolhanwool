...
- 2~4인 팀에서 **담당 경계를 먼저 긋고**, 서로의 작업이 서로를 막지 않는 구조로 나눠 일합니다.
...
2인 · 3인 · 4인 팀에서 각각 다른 자리를 맡아봤습니다. 팀 작업에서 사고가 나는 지점은 대개 실력이 아니라
**누가 어디까지 만지는가가 흐릿할 때**라, 경계를 코드와 문서 양쪽에 먼저 그어 둡니다.

| | 하는 일 | 실제 사례 |
|---|---|---|
| **담당을 코드 구조로 나눈다** | 모듈 · 폴더 단위로 소유자를 정해, 각자 남의 작업을 기다리지 않고 병렬로 굴러가게 만든다 | **열받는 게임** — 인식 계층을 게임과 완전히 분리해, 제가 파이프라인을 고치는 동안 팀원은 게임 로직을 계속 진행 |
| **남의 코드에 손대지 않고 잇는다** | 팀원 담당 스크립트를 직접 참조하는 대신 우회 경로를 만들어, 상대 작업이 바뀌어도 서로 깨지지 않게 한다 | **Wizard War** — 미니맵이 트로피 내부를 직접 참조하지 않도록 분리 (트로피는 다른 팀원 담당) |
| **내 몫이 아닌 것은 내 몫이 아니라고 쓴다** | 케이스 문서마다 **담당이 아닌 영역을 표로 명시**하고, 공동 작업 파일은 `git blame` 으로 **내가 쓴 메서드만** 집계한다 | [카피바라](https://hanwoolhanwool.github.io/portfolio/capybara-portfolio.html) · [열받는 게임](https://hanwoolhanwool.github.io/portfolio/sam-i-bang-portfolio.html) · [Wizard War](https://hanwoolhanwool.github.io/portfolio/wizard-war-portfolio.html) 케이스 문서 3편 |
| **마감 앞에서는 경계를 넘는다** | 제출 · 출시 직전에는 담당 구분 없이 비어 있는 자리를 메운다 | **열받는 게임** — 최종 빌드의 연출 · 타이밍 · 수치 마무리를 담당 밖까지 맡음 |
| **끊긴 사람을 복구한다** | 누가 빠져도 남은 사람의 플레이가 멈추지 않도록 세션 흐름을 짠다 | **Wizard War** — 방장이 로딩 중 나가도 새 마스터가 판정을 이어받아 전원이 로딩 화면에 갇히지 않게 처리 |

<sub><b>카피바라 몸짱 만들기</b>는 2인 팀에서 기획 · 개발 총괄을 맡아 출시와 라이브 개선까지, <b>열받는 게임</b>은 3인 팀에서 입력 파이프라인 전 구간을, <b>Wizard War</b>는 4인 팀에서 네트워크 · 세션 영역을 담당했습니다.</sub>
...
- 커밋 531 · 스크립트 308개 · **EditMode 테스트 753개 통과**로 최적화 후 회귀 차단
...
- 토스 앱인 **인기 순위 87위** · 커밋 68 · 스크립트 44/46 · **자체 코드 90.1%**
...
**성과**
- 프레임당 **21~468개 랜드마크**를 `List<List<float>>`로 새로 만들던 경로를 **손 데이터 객체 풀링**으로 바꿔 — 값만 덮어쓰고 구조는 재사용, **프레임당 할당을 사실상 0으로** 축소
- 워커 → 메인 스레드 브릿지에 **큐 상한 30** — 메인 스레드가 밀리면 오래된 프레임부터 버려, **입력 지연이 누적되며 손과 화면이 어긋나는 현상 차단**
- 비싼 **룰 매칭은 새 데이터가 올 때만(약 30Hz)**, 시간 디바운스는 **매 프레임(60Hz 이상)** 으로 주기를 분리해 연산 낭비 제거
- 종횡비 캐시 · 신뢰도 게이트 · OneEuroFilter로 **떨림과 반응 지연을 한 필터로 동시 해결**
- 커밋 197 · 전담 모듈 14,873줄 · **자체 코드 49.1%** — 인식 계층이 게임을 모른 채 **단독 테스트 가능**
